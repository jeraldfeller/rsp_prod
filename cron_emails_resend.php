<?php
// Resend a day's worth of emails, with an added apology
//
// To use this script:
//
// (1) Get SSH access to production (use CPanel to manage SSH keys at HostDNX)
// (2) Update the $resend_date variable to whatever date needs to be resent
// (3) Make sure the timing for the resend is good (not too close to the normal send)
// (4) Run this script with the command:  php -q cron_emails_resend.php
//
//
	...
	if ($resend_date) {
		...
		$sql = "SELECT * FROM " . TABLE_EMAILS_DUE . " WHERE date = '$resend_date' ORDER BY user_id ";
			...
			foreach($database->fetch_array($query) as $result){
				...
				if ($database->num_rows($order_query) > 0) {
					...
					foreach($database->fetch_array($order_query) as $order_result){
						...
						if ($live == false) echo "<tr><td width='30%'>Date for Removal: </td><td width='70%'>\n";
        ...
        foreach($user_id as $y => $uid) {
			if ($message[$y] != '') {
				...
				$new_message = "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\">
					<tbody>
						<tr>
                            <td>Greetings from Realty SignPost. Below is a corrected list of addresses where you currently have a signpost installed.
                            You are receiving this email because an earlier Signpanel Inventory &amp; Active Signpost Summary email which was sent out 
                            may have contained some incorrect data.  We apologize for this mistake and any confusion it might have caused.
                            This updated list includes the date the signpost was installed, and the scheduled removal date.
                            If you no longer need the signpost at one of the addresses listed, please reschedule the removal order for that address at 
		...
		//send email to Ryan
		$msg = "Total number of emails resent today (not including extra agent email addresses): " . $emails_sent;

		$subject = 'Inventory Email Resend Stats';
