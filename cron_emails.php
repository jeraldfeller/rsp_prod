	...
	if ($date_today == 1) { //get all active orders/customers
        ...
        foreach($user_id as $y => $uid) {
			if ($message[$y] != '') {
				...
				$query_agents_sql = "SELECT u.firstname, u.lastname, ag.name, ag.address FROM users_description u JOIN users ur ON ur.user_id = u.user_id LEFT JOIN agencys ag ON ag.agency_id = ur.agency_id WHERE u.user_id = ".$uid;
				$query_agents = $database->query($query_agents_sql);
				foreach($database->fetch_array($query_agents) as $agent_info){
					$full_name = $agent_info['firstname']." ".$agent_info['lastname'];
					$agency_name = $agent_info['name'];
					$string_ad = trim(preg_replace('/\s\s+/', ' ', $agent_info['address']));
					$agency_address = $string_ad;
				}
