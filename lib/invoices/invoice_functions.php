...
function get_invoice_items($month, $year, $user_id, $agency_id) 
{
	...
	$s = "
	SELECT
    DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(COALESCE(o.date_completed, a.date_added)), '+07:40', '-05:00'), '%c/%e/%Y') AS `order_datecompleted`,
	...
	WHERE 
    (
        (o.date_completed >= UNIX_TIMESTAMP(CONVERT_TZ('{$start_year}-{$start_month}-01', '-05:00', '+07:40')) AND
            o.date_completed < UNIX_TIMESTAMP(CONVERT_TZ('{$end_year}-{$end_month}-01', '-05:00', '+07:40')))
        OR
        (a.reference_id = 0 AND a.year_added >= 2014 AND 
            UNIX_TIMESTAMP(CONCAT(a.year_added, '-', a.month_added, '-01 23:00:00')) >= UNIX_TIMESTAMP(CONVERT_TZ('{$start_year}-{$start_month}-01', '+08:00', '-05:00')) AND 
            UNIX_TIMESTAMP(CONCAT(a.year_added, '-', a.month_added, '-01 23:00:00')) < UNIX_TIMESTAMP(CONVERT_TZ('{$end_year}-{$end_month}-01', '+08:00', '-05:00')))
    )
    ";
	...
	foreach($q as $r)
	{
		//var_dump("RECORD"); echo '<pre>'; var_dump($r); echo '</pre>';
