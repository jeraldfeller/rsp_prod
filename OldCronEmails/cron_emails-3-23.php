<?php
	...
	$limit = 35; //if we're limiting no of records (set to 0 if no limit)
	...
	if ($date_today == 1) { //get all active orders/customers
		...
		$sql = "SELECT DISTINCT o.user_id FROM " . TABLE_ORDERS . " as o, " . TABLE_USERS . " as u WHERE o.user_id = u.user_id AND o.address_id NOT IN (SELECT address_id FROM " . TABLE_ORDERS . " WHERE (order_type_id = 3 AND order_status_id = 3) OR (order_type_id = 3 AND order_status_id = 4))";
