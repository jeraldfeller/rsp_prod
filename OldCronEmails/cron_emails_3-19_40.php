<?php
	...
	$limit = 40; //if we're limiting no of records (set to 0 if no limit)
	...
	if ($date_today == 1) { //get all active orders/customers
		...
		$sql = "SELECT DISTINCT o.user_id FROM " . TABLE_ORDERS . " as o, " . TABLE_USERS . " as u WHERE o.user_id = u.user_id AND o.address_id NOT IN (SELECT address_id FROM " . TABLE_ORDERS . " WHERE order_type_id = 3 AND order_status_id = 3)";
		...
		$message->send('', 'realtysp@yahoo.com', EMAIL_FROM_NAME, EMAIL_FROM_ADDRESS, $subject);
	...
	for($i=1;$i<count($headers)+1;$i++) {
		...
		$msg = $mailbox->getMessage($i);
		if ((strpos($msg['subject'],'fail') == true) || (strpos($msg['body'],'fail') == true)) {
			$start = strpos($msg['body'],'To:');
			$end = strpos($msg['body'],'Subject:');
			$length = $end-$start;
			$email = substr($msg['body'],$start,$length);
			$email_msg .= $email . '\n';
