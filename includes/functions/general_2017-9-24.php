...
function tep_generate_post_type_pulldown_menu($name, $selected = '', $params = '', $zip4 = '', $required = 1) {
    ...
    //$query = $database->query("select count(e.equipment_id) as count, e.equipment_id, e.name from " . TABLE_EQUIPMENT . " e, " . TABLE_EQUIPMENT_ITEMS . " ei, " . TABLE_WAREHOUSES . " w left join " . TABLE_WAREHOUSES_TO_AREAS . " wta on (w.warehouse_id = wta.warehouse_id and ((wta.zip_4_first_break_start < '" . $zip4_explode[0] . "' and wta.zip_4_second_break_start > '" . $zip4_explode[0] . "') or ((wta.zip_4_first_break_start <= '" . $zip4_explode[0] . "' and wta.zip_4_second_break_start >= '" . $zip4_explode[0] . "') && (wta.zip_4_first_break_end <= '" . $zip4_explode[1] . "' and wta.zip_4_second_break_end >= '" . $zip4_explode[1] . "')))) where e.equipment_type_id = '" . $equipment_type_id . "' and e.equipment_id = ei.equipment_id and ei.equipment_status_id = '0' and ei.warehouse_id = w.warehouse_id and (w.availability = '0' or wta.warehouse_to_area_id is not NULL) group by e.equipment_id order by e.name");
	//echo "select count(e.equipment_id) as count, e.equipment_id, e.name from " . TABLE_EQUIPMENT . " e, " . TABLE_EQUIPMENT_ITEMS . " ei, " . TABLE_WAREHOUSES . " w left join " . TABLE_INSTALLATION_AREAS . " ia on (w.warehouse_id = ia.warehouse_id) left join " . TABLE_INSTALLATION_COVERAGE_AREAS . " ica on (ica.installation_area_id = ia.installation_area_id and ((ica.zip_4_first_break_start < '" . $zip4_explode[0] . "' and ica.zip_4_second_break_start > '" . $zip4_explode[0] . "') or ((ica.zip_4_first_break_start <= '" . $zip4_explode[0] . "' and ica.zip_4_second_break_start >= '" . $zip4_explode[0] . "') && (ica.zip_4_first_break_end <= '" . $zip4_explode[1] . "' and ica.zip_4_second_break_end >= '" . $zip4_explode[1] . "')))) where e.equipment_type_id = '" . $equipment_type_id . "' and e.equipment_id = ei.equipment_id and ei.equipment_status_id = '0' and ei.warehouse_id = w.warehouse_id and (w.availability = '0' or ica.installation_area_id is not NULL) group by e.equipment_id order by e.name";
	//die();

	$query = $database->query("select count(e.equipment_id) as count, e.equipment_id, e.name from " . TABLE_EQUIPMENT . " e, " . TABLE_EQUIPMENT_ITEMS . " ei, " . TABLE_WAREHOUSES . " w left join " . TABLE_INSTALLATION_AREAS . " ia on (w.warehouse_id = ia.warehouse_id) left join " . TABLE_INSTALLATION_COVERAGE_AREAS . " ica on (ica.installation_area_id = ia.installation_area_id and (((ica.zip_4_first_break_start < '" . $zip4_explode[0] . "') OR ( ica.zip_4_first_break_start = '" . $zip4_explode[0] . "' and ica.zip_4_first_break_end <= '" . $zip4_explode[1] . "')) AND ((ica.zip_4_second_break_start > '" . $zip4_explode[0] . "') OR (ica.zip_4_second_break_start = '" . $zip4_explode[0] . "' AND ica.zip_4_second_break_end >= '" . $zip4_explode[1] . "')))) where e.equipment_type_id = '" . $equipment_type_id . "' and e.equipment_id = ei.equipment_id and ei.equipment_status_id = '0' and ei.warehouse_id = w.warehouse_id and (w.availability = '0' or ica.installation_area_id is not NULL) group by e.equipment_id order by e.name");
	...
	/*echo "select count(e.equipment_id) as count, e.equipment_id, e.name from " . TABLE_EQUIPMENT . " e, " . TABLE_EQUIPMENT_ITEMS . " ei, " . TABLE_WAREHOUSES . " w left join " . TABLE_INSTALLATION_AREAS . " ia on (w.warehouse_id = ia.warehouse_id) left join " . TABLE_INSTALLATION_COVERAGE_AREAS . " ica on (ica.installation_area_id = ia.installation_area_id and ((ica.zip_4_first_break_start < '" . $zip4_explode[0] . "' OR ( ica.zip_4_second_break_start = '" . $zip4_explode[0] . "' and ica.zip_4_second_break_end <= '" . $zip4_explode[1] . "')) AND (ica.zip_4_second_break_start > '" . $zip4_explode[0] . "' OR (ica.zip_4_second_break_start = '" . $zip4_explode[0] . "' AND ica.zip_4_second_break_end >= '" . $zip4_explode[1] . "')))) where e.equipment_type_id = '" . $equipment_type_id . "' and e.equipment_id = ei.equipment_id and ei.equipment_status_id = '0' and ei.warehouse_id = w.warehouse_id and (w.availability = '0' or ica.installation_area_id is not NULL) group by e.equipment_id order by e.name";

	echo '<br><br><pre>'; print_r($array);	*/
...
function tep_draw_aom_agent_pulldown_bgdn_pending_service_calls($name = '', $selected = '', $aom = '', $params = '', $array = array()) {
    ...
    $return = '';
    $agency_id = tep_fetch_order_manager_agency($aom);
    $query = $database->query("select u.user_id, u.agent_id, ud.firstname, ud.lastname from " . TABLE_USERS . " u, " . TABLE_USERS_DESCRIPTION . " ud, " . TABLE_USERS_TO_USER_GROUPS . " utug where utug.user_group_id = '1' and utug.user_id = u.user_id and u.agency_id = '7572' and u.user_id = ud.user_id and u.user_id IN (SELECT user_id FROM " . TABLE_ORDERS . " WHERE `order_type_id`='2' and `order_completed_status`='0' ) and u.active_status = '1' order by ud.firstname");
...
function tep_draw_aom_agent_pulldown_bgdn_pending_removals($name = '', $selected = '', $aom = '', $params = '', $array = array()) {
    ...
    $return = '';
    $agency_id = tep_fetch_order_manager_agency($aom);
    $query = $database->query("select u.user_id, u.agent_id, ud.firstname, ud.lastname from " . TABLE_USERS . " u, " . TABLE_USERS_DESCRIPTION . " ud, " . TABLE_USERS_TO_USER_GROUPS . " utug where utug.user_group_id = '1' and utug.user_id = u.user_id and u.agency_id = '7572' and u.user_id = ud.user_id and u.user_id IN (SELECT user_id FROM " . TABLE_ORDERS . " WHERE `order_type_id`='3' and `order_completed_status`='0' ) and u.active_status = '1' order by ud.firstname");
...
function tep_equipment_group_has_items($group_id, $user_id, $warehouse_data = array(), $address_id = '', $exclude_array = array(), $include_out = false) {
    ...
    $query = $database->query("select equipment_group_answer_id, install_equipment_id, remove_equipment_id from " . TABLE_EQUIPMENT_GROUP_ANSWERS . " where equipment_group_id = '" . $group_id . "' and service_level_id <= '" . $service_level_id . "'");
    while ($result = $database->fetch_array($query)) {
...
function tep_fetch_agent_data($agent_id) {
    ...
    while ($phone_result = $database->fetch_array($phone_query)) {
...
//					echo 'found<br>';
        ...
        while ($optional_result = $database->fetch_array($optional_query)) {
...
function tep_fetch_installation_availability($zip4_code) {
    ...
    if (tep_zip4_is_valid($zip4_code)) {
        ...
        $query = $database->query("select count(installation_coverage_area_id) as count from " . TABLE_INSTALLATION_COVERAGE_AREAS . " where ((zip_4_first_break_start < '" . $zip4_break[0] . "') or (zip_4_first_break_start = '" . $zip4_break[0] . "' and zip_4_first_break_end <= '" . $zip4_break[1] . "')) and ((zip_4_second_break_start > '" . $zip4_break[0] . "') or (zip_4_second_break_start = '" . $zip4_break[0] . "' and zip_4_second_break_end >= '" . $zip4_break[1] . "')) limit 1");
        $result = $database->fetch_array($query);
        if ($result['count'] > 0) {
            $return = true;
        }
...
function tep_format_order_email($order_id, $template, $aom_id = "", $applied_credit = 0, $is_installer='') {
    global $database;
...
//die();
	...
	if(!empty($is_installer)){
		//$email_template->send_email('varinderrana.vr@gmail.com', $result['firstname'] . ' ' . $result['lastname']);
		$email_instal = trim($is_installer);
		$email_template->send_email($email_instal, $result['firstname'] . ' ' . $result['lastname']);
	}else{
...
function get_account_id($agency_id) {
    ...
    $get_aid = $database->query("
              ...
              LIMIT 1
      ");
    while ($r = $database->fetch_array($get_aid)) {
    ...
    if(!$this_account_id) {
        ...
        while($r = $database->fetch_array($q)) {
            ...
            if(!empty($aid)) {
                ...
                while($r2 = $database->fetch_array($q2)) {
