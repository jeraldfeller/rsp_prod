...
function tep_draw_user_pulldown($name = '', $selected = '', $params = '', $array = array(), $ignore_user = '') {
    global $database;
    $return = '';
    $query = $database->query("select user_id, email_address from " . TABLE_USERS . " where users_status = '1' " . ((!empty($ignore_user)) ? " and user_id != '" . $ignore_user . "' " : '') . " order by email_address");
    while ($result = $database->fetch_array($query)) {
...
function tep_generate_available_equipment_string($order_type, $service_level, $user_id, $selected, $zip4, $address_id = '', $edit = false, $show_default = true, $exclude_array = array(), $show_price = true, $show_prevent = true) {
    global $database;
        ...
        while ($optional_result = $database->fetch_array($optional_query)) {
            ...
            if ($available_equipment_count < 1) {
               ...
               $optional_result['inventory_level'] = "";
                ...
                $optional_result['inventory_level'] = " - <strong>({$available_equipment_count} available in this service area)</strong>";
            }
...
function tep_count_installer_orders($installer_id, $day, $month, $year, $order_type_id = '', $order_status_id = '', $state_id = '', $limit = true) {
    global $database;
    $string = '';
    if (!empty($order_type_id)) {
        $string .= " and o.order_type_id = '" . $order_type_id . "'";
...
function tep_create_button_submit($button_name, $alt = '', $params = '') {
    ...
    if (is_file(DIR_IMAGES . $image_string)) {
        ...
        $return_string = '<input type="submit" value="' . $alt . '" ' . $params . ' />';
    }
    return $return_string;
}
