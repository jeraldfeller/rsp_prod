			...
			global $pdf, $page, $y, $midnight_future, $user;
				...
				//Do the header + footer.
					if ($page > 1) {
						...
						//$pdf->addText(750,$y-3,9, '<i>Continued on next page...</i>');
						change_color(BLACK);
						$pdf->ezNewPage();
						change_color(BLACK);
					}
				...
				$pdf->addText(390,$y,10,'Service Calls: ' . tep_count_installer_orders($user->fetch_user_id(), date("d", ($midnight_future-1)), date("n", ($midnight_future-1)), date("Y", ($midnight_future-1)), '2', '2', '', false));
				$pdf->addText(740,$y,10,'Removals: ' . tep_count_installer_orders($user->fetch_user_id(), date("d", ($midnight_future-1)), date("n", ($midnight_future-1)), date("Y", ($midnight_future-1)), '3', '2', '', false));
				...
				$pdf->ezStartPageNumbers(810,26,12, 'left', 'Page {PAGENUM} of {TOTALPAGENUM}');
					if ($add_box) {
						change_color(GREY);
						$pdf->filledRectangle(28,$y - 40,785,50);
						change_color(BLACK);
						...
						$pdf->addText(170,$y,8,"Agent/Agency");
						$pdf->addText(170,$y-9,8,"Svc Level");
						$pdf->addText(170,$y-18,8,"Preferences");
						$pdf->addText(440,$y,8,"Address");
						$pdf->addText(440,$y-9,8,"ADC map coord");
						$pdf->addText(440,$y-18,8,"Directions");
						$pdf->addText(440,$y-27,8,"Agent Comments");
						$pdf->addText(690,$y,8,"Contact #s");
						...
						$pdf->filledRectangle(164,$y - 40,2,50);
						$pdf->filledRectangle(434,$y - 40,2,50);
						$pdf->filledRectangle(684,$y - 40,2,50);
						//$pdf->ezSetY(60);
	...
	$loop = 0;
		...
		while($result = $database->fetch_array($query)) {
			...
			$y -= 8;
				...
				if ($loop > 0) {
					change_color(BLACK);
					$pdf->filledRectangle(48,$y - 1,745,1);
					change_color(BLACK);
					$y -= 14;
				}
			...
			$order_description = '<b>' . $result['house_number'].' ' .$result['street_name'].'</b><br>'.$result['city'].' '.$result['state_name'].' '.$result['zip4'].'<br />'.((!empty($adc)) ? $adc . '<br>' : '') . $result['cross_street_directions'].'<br />'.$result['special_instructions'].'<br />'.$result['admin_comments'] . (($result['address_post_allowed'] == '0') ? '<br><b>Posts may not be allowed at this address.</b>' : '');
				if ($order_data['order_type_id'] == '2') {
					...
					//$order_description .= str_replace('&nbsp;', ' ', $string);
				}
			$final_y = $y;
			$temp_y = $y;
			...
			//echo strip_tags(str_replace(array('<br>', '<br />'), "\n", str_replace('<img src="images/pixel_trans.gif" height="3" width="1">', '<br>', tep_create_view_equipment_string($order_data['optional'], true)))) . '<br><br><br>';
			$explode = explode("\n", strip_tags(str_replace(array('<br>', '<br />'), "\n", str_replace('<img src="images/pixel_trans.gif" height="3" width="1">', '<br>', tep_create_view_equipment_string($order_data['optional'], true)))));
				for ($n = 0, $m = count($explode); $n < $m; $n++) {
					$explode[$n] = trim($explode[$n]);
						if (!empty($explode[$n])) {
							$pdf->ezSetY($temp_y);
							$temp_y = $pdf->ezText($explode[$n],9,array('justification'=>'left', 'aright' => 130, 'aleft' => 30));
						}
				}
			//$temp_y = $pdf->ezText(str_replace(strip_tags(str_replace(array('<br>', '<br />'), "\n", str_replace('<img src="images/pixel_trans.gif" height="3" width="1">', '<br>', tep_create_view_equipment_string($order_data['optional'], true)))),9,array('justification'=>'left', 'aright' => 130, 'aleft' => 30));
			$temp_y -= 10;
			...
			$temp_y = $y;
			$pdf->ezSetY($temp_y);
			$temp_y = $pdf->ezText($agent_data['firstname'] .' ' . $agent_data['lastname'] . ' / ' . $agent_data['name'],9,array('justification'=>'left', 'aright' => 430, 'aleft' => 168));
			$temp_y -= 10;
			$pdf->addText(168,$temp_y,9,$result['service_level_name']);
				if (tep_agent_has_preferences($result['user_id'], $result['order_type_id'])) {
					$pdf->ezSetY($temp_y);
					$temp_y = $pdf->ezText(str_replace(array('<br>', '<br />'), "\n", tep_create_agent_preferences_string($result['user_id'], $result['order_type_id'])),9,array('justification'=>'left', 'aright' => 430, 'aleft' => 168));
				}
				if ($temp_y < $final_y) {
					$final_y= $temp_y;
				}
			...
			$temp_y = $y;
			$pdf->ezSetY($temp_y);
			$temp_y = $pdf->ezText(str_replace(array('<br>', '<br />'), "\n", $order_description),9,array('justification'=>'left', 'aright' => 660, 'aleft' => 440));
			...
			$temp_y = $y;
				for ($n = 0, $m = count($agent_data['phone_numbers']); $n < $m; $n++) {
					$pdf->ezSetY($temp_y);
					$temp_y = $pdf->ezText($agent_data['phone_numbers'][$n] . (($n == 0) ? ' - Cell' : (($n == 2) ? ' - Fax' : '')),9,array('justification'=>'left', 'aright' => 800, 'aleft' => 690));
				}
				if ($temp_y < $final_y) {
					$final_y= $temp_y;
				}
