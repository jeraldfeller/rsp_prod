 ...
 */
	...
	//echo 'adding ' . $file . ' - ' . $name . ' - ' . $c_type . '<br>';
 ...
 */
    ...
    function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
      ...
      if (!isset($xtra_headers)) {
		...
		if ((SEND_EMAILS == 'true' && $this->test_flag == false)) {
		
		  if (EMAIL_TRANSPORT == 'smtp') {

				if (($to_addr != ADMIN_EMAIL) && ($to_addr != INFO_EMAIL)) {
					//mail(ADMIN_EMAIL, $subject . ' sent to ' . $to_addr, $this->output, 'From: ' . $from . $this->lf . 'To: ' . $to . $this->lf . implode($this->lf, $this->headers) . $this->lf . implode($this->lf, $xtra_headers));
				}
			//return mail($to_addr, $subject, $this->output, 'From: ' . $from . $this->lf . 'To: ' . $to . $this->lf . implode($this->lf, $this->headers) . $this->lf . implode($this->lf, $xtra_headers));
		  } else {
              echo $to_addr . '<BR>';

				//if ($to_addr != INFO_EMAIL) {
				//	mail(ADMIN_EMAIL, $subject . ' sent to ' . $to_addr, $this->output, 'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers));
			//	}
            ...
            // Don't send, but send to error_log so they can be viewed
