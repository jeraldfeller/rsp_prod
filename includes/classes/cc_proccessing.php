...
/*
LOGIN INFO
	https://morpheus.hostdnx.com:2083	realtysi
*/

if (getenv("SERVER_MODE") == "TEST") {
    define('EPNACCOUNT','05971'); // Testing value.
    define('RESTRICTKEY','yFqqXJh9Pqnugfr'); // Testing value.
} else {
    define('EPNACCOUNT','0512949'); //Production value
    define('RESTRICTKEY','Myhp3uPNQceHSKg');
}
...
class cc_proccessing {
	...
	function pre_transaction($cardnumber, $cardname, &$errornumber, &$errortext) {
		...
		// every four digits.
		$cardexp = '~^([0-9]{4})\s?([0-9]{4})\s?([0-9]{4})\s?([0-9]{1,4})$~smUi';
		$hasMatch=preg_match($cardexp,$cardnumber, $matches);
		if($hasMatch===false or $hasMatch===0)
		{
			$errornumber = 2;
			$errortext = $ccErrors [$errornumber];
			return false;
		...
		$exp = '~^' . $prefix[$i].'~';
		if (preg_match($exp,$cardNo)!==false) {
			$PrefixValid = true;
			break;
		}
		}
    ...
    function log_transaction() {
        ...
        //$user_id = $this->fetch_query_variable('order_user_id');  // The order's user_id
        $user_id = $user->fetch_user_id();                          // The session's user_id

        $exploded = explode(":", $this->fetch_query_variable('order_description'));
        $reason = $exploded[0];
        if (count($exploded) > 1) {
            $details = $exploded[1];
        } else {
        ...
        // It would be nice to have an order_id, but we don't since the CC is processed before the order_id is created.
