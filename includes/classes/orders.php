<?php
	class orders {
		...
		var $order_insert_type_id;
			...
			function __construct($action = 'fetch', $id = '', $data = array(), $user_id = '', $change_address_status = true, $order_insert_type_id = '1')
			{
				global $user;
			...
			function update_order() {
                ...
                foreach($database->fetch_array($check_reschedule_query) as $check_reschedule_row){
				...
				if ($result['order_type_id'] == '1') {
					...
					// echo "new_total->".$new_total." old_total->".$old_total."<br/>";
					...
					// echo "update " . TABLE_ORDERS . " set equipment_cost = '{$equipment_cost}', order_total = '{$new_total}', discount_cost = '{$discount_cost}', base_cost = '{$base_cost}', extended_cost = '{$extended_cost}', extra_cost = '{$extra_cost}', deposit_cost = '{$deposit_cost}' where order_id = '{$this->id}' limit 1<br/>";
            ...
            function cancel_order() {
                ...
                if (($order_type_id == ORDER_TYPE_INSTALL) || ($order_type_id == ORDER_TYPE_SERVICE)) {
                    ...
                    foreach($database->fetch_array($equery) as $result){
			...
			function insert_order($update_status = true) {
				...
				if ($this->discount_cost > 0) {
					...
					if (!empty($promo_id_result)) {
						...
						$database->query("insert into " . TABLE_PROMOTIONAL_CODES_TO_USERS . " (promotional_code_id, user_id, date_added) values ('" . $promo_id_result['promotional_code_id'] . "', '" . $this->user_id . "', '" . time() . "')");
					}
                }
            ...
            function fetch_order_total($zip4 = "") {
                ...
                $order_type_id = $this->fetch_data_item('order_type_id', '1');
								// echo "First get the base cost 4<br/>";
                ...
                $cost += $this->base_cost;
								if($this->base_cost==0){
									$this->base_cost = $this->create_base_order_cost_aom() * $this->fetch_data_item('number_of_posts', '1');
					        $cost += $this->base_cost;
									// echo "this->id->".$this->id."<br/>";
								}
								// echo "1 First get the base cost ".$this->base_cost ." cost->".$cost."<br/>";
			...
			function fetch_order_total_sc() {
				...
				$free = false;
				// echo "First get the base cost 2<br/>";
				...
				if ($sc_reason == '2') {
					$this->equipment_cost = $this->create_order_equipment_cost();
					// echo "First get the base cost 3<br/>";
					...
					if ($free) {
                        ...
                        $this->equipment_cost = max(0, 5 * (count($install_equipment) - 1));
												// echo "First get the base cost 5<br/>";
			...
			function create_base_order_cost() {
				global $user, $database;
				// echo "this->user_id->".$this->user_id."<br/>";
				...
				// echo "select installation_cost from " . TABLE_ORDER_TYPES . " where order_type_id = '" . $this->fetch_data_item('order_type_id', false) . "' limit 1<br/>";
				$result = $database->fetch_array($query);
				...
				// echo "service_level_cost->".$service_level_cost."<br/>";
				return $service_level_cost;
			}

			function create_base_order_cost_aom() {
				global $user, $database;

				$query = $database->query("Select * from ".TABLE_ORDERS." where `order_id`=".$this->id);
				$result = $database->fetch_array($query);
				$service_level_id = tep_get_service_level_id($result['user_id']);
				if ($this->fetch_data_item('order_type_id', '1') == '3') {
					return 0;
				}
				$query = $database->query("select installation_cost from " . TABLE_ORDER_TYPES . " where order_type_id = '" . $this->fetch_data_item('order_type_id', false) . "' limit 1");
			...
			function create_order_equipment_cost() {
                ...
                $number_of_posts = $this->fetch_data_item('number_of_posts', 1);
				#echo tep_fetch_equipment_cost($optional, $service_level_id, $address_id, $order_type_id); Commented By Mukesh to remove zero(0)
