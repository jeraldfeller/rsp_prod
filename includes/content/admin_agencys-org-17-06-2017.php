            ...
            for ($i = 1; $i <= 4; $i++) {
				...
				if (!$error->get_error_status('admin_agencys')) {
						if ($page_action == 'edit') {
                            ...
                            $database->query("insert into " . TABLE_AGENCYS . " (name, office, service_level_id, billing_method_id, address, contact_name, contact_phone, parent_agency_id, discount_type, discount_amount, agency_status_id, auto_remove_period) values ('" . $name . "', '" . $office . "', '" . $service_level_id . "', '" . $billing_method_id . "', '" . addslashes($address) . "', '" . $contact_name . "', '" . $contact_phone . "', '" . $parent_agency_id . "', '" . $discount_type . "', '" . $discount_amount . "', '1', " . $auto_remove_period .")");
		...
		<td width="100%" valign="top">
			...
			<?php
							...
							$query = $database->query($listing_split->sql_query);
								while($result = $database->fetch_array($query)) {
									...
									if ( ($active_agents == 0) || ($total_agents == $inactive_agents) || ($all_inactive) ) {
										...
										while($result_misc = $database->fetch_array($misc_query)) {
				...
				<?
					...
					if (!empty($aID)) {
						//Edit
						$agency_query = $database->query("select name, office, service_level_id, billing_method_id, address, contact_name, contact_phone, parent_agency_id, discount_type, discount_amount, auto_remove_period from " . TABLE_AGENCYS . " where agency_id = '" . $aID . "' limit 1");
                        ...
                        $i = 1;
                        while($agency_email_result = $database->fetch_array($agency_email_query)) {
                            $var = "email" . $i;
						...
						//Add
						$agency_result = array('name' => '', 'office' => '', 'service_level_id' => '', 'billing_method_id' => '', 'address' => '', 'contact_name' => '', 'contact_phone' => '', 'parent_agency_id' => '', 'discount_type' => '', 'discount_amount' => '', 'auto_remove_period' => '');
                        for ($i = 1; $i <= 4; $i++) {
                            foreach(array("email" . $i, "use_email" . $i, "email" . $i . "checked") as $var) {
                                $$var = "";
                            }
			...
			<table width="250" cellspacing="0" celpadding="0" class="pageBox">
				...
				<tr>
					<form action="<?php echo FILENAME_ADMIN_AGENCYS; ?>" method="get">
						...
						<tr>
							<td class="main">Show Agencies with Status: <select name="search_status"><option value=""<?php echo (($search_status == '') ? ' SELECTED' : ''); ?>>Any</option><option value="0"<?php echo (($search_status == '0') ? ' SELECTED' : ''); ?>>Active</option><option value="1"<?php echo (($search_status == '1') ? ' SELECTED' : ''); ?>>Inactive</option></option></td>
						</tr>
                        <tr>
