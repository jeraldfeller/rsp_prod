<?php // Updated 1/17/2013 brad@brgr2.com Removed option for users to edit/remove scheduled or complete orders.
    ...
    if (!empty($oID))
	{
		...
		// echo "2 order id->".$oID."<br/>";
	if ($page_action == 'edit_confirm')
	{
		//Do all the checking.
//echo "order-data->".$order_data['order_type_id']."<br/>";
		...
		else
		{
			...
			if ($date_schedualed < strtotime("today + " . SCHEDULE_DEFAULT_OFFSET . " days")) {
				$fail = true;
...
// echo 'Total->'.$total.'<br/>';
			...
			// echo $new_total;
			if ($new_total > $total && tep_fetch_agent_billing_method_id($user->fetch_user_id()) == BILLING_METHOD_CREDIT)
			{
				// die('aaaa');
		...
		// if (true)
		{
			...
			if (!empty($card_request))
			{
				...
				$cc_billing_zip = tep_fill_variable('cc_billing_zip', 'post', tep_fill_variable('cc_billing_zip', 'session'));
						...
						//
						// var_dump($oID);
						// $page["PAGE_TITLE"] = "Search Order History";
						// $page["PAGE_KEYWORDS"] = "";
						// $page["PAGE_DESCRIPTION"] = "Allows agent to view all orders.";
						// $page["PAGE_NAME"] = "Search Order History";
						// $twig = new Twig_Environment($loader, array(
						// 						'cache' => false,//DIR_INCLUDES.'twig_templates/cache',
						// 					));
						// var_dump($twig);
						// echo "hey1 <br/>";
						echo $twig->render('aom/edit_order0.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'vars'=>$vars));
// var_dump($vars);
						...
						$vars['return_page'] = isset($_GET['return_page']) ? $_GET['return_page'] : "aom_active_addresses.php";
						$vars['page_action'] = $page_action;
						$vars['order_data']['oID'] = $oID;
						$vars['order_data']['aID'] = $aID;
						// echo "hey<br/>";
