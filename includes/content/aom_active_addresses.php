...
*/
if($page_action=='order_edit' | $page_action=='edit_confirm'| $page_action=='delete_order' | $page_action=='delete_confirm') {
 // echo 'string';
 include 'aom_order_view.php';
} else {
			...
			$vars['listing_split'] = $listing_split;
						...
						$query = $database->query($listing_split->sql_query);
						    foreach($database->fetch_array($query) as $result){
								...
								$install_date_query = $database->query("select order_id, date_schedualed, order_status_id from " . TABLE_ORDERS . " where address_id = '" . $result['address_id'] . "' and order_type_id = '1' limit 1");
								...
								if ($result['install_date']['order_status_id'] == '1' || $result['install_date']['order_status_id'] == '2') {
									// echo "string->".$result['install_date']['order_id']."<br/>";
							...
							// echo $user->fetch_user_id()." <-user_id<br/>";
					$pulldowns = array(
