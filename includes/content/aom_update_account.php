...
//error_reporting(0);
$user_query = $database->query("select email_address, billing_method_id, service_level_id, agency_id from " . TABLE_USERS . " where user_id = '" . $user->fetch_user_id() . "' limit 1");
...
if (!empty($update_account) && ($update_account == 'update')) 
{
	...
	if (!$error->get_error_status('account_update')) 
	{
        ...
        $database->query("update " . TABLE_USERS . " set last_modified = '" . time() . "', email_address = '" . $form['email_address'] . "' where user_id = '" . $user->fetch_user_id() . "' limit 1");
...
/**/
	...
	$vars['update_success'] = $update_success;
	//print_r($vars['form']['alternate_result']);

	echo $twig->render('aom/aom_account_update.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'vars'=>$vars));
