...
//echo $agent_id;
    ...
    $personal = tep_is_personal_equipment($agent_id, $user->agency_id);
	...
	$vars['form'] = $form;
		...
		if (($order_type == ORDER_TYPE_INSTALL)) {
			...
			//echo tep_generate_available_equipment_string($order_type, tep_get_service_level_id($agent_id), $agent_id, $optional, tep_fill_variable('zip4', 'session'), tep_fill_variable('address_id', 'session'));
						//Install.
				
					$vars['equipment_array'] = tep_generate_available_equipment_string_bgdn($order_type, tep_get_service_level_id($agent_id), $agent_id, $optional, tep_fill_variable('zip4', 'session'), tep_fill_variable('address_id', 'session'));
					...
					echo $twig->render('aom/aom_order_create_special.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'vars'=>$vars));
					...
					elseif ($order_type == ORDER_TYPE_SERVICE) {
						...
						if (!empty($sc_reason)) {
											if ($sc_reason == '1') {
													...
													$query = $database->query("select eita.equipment_id, e.name as equipment_name from " . TABLE_EQUIPMENT_ITEMS_TO_ADDRESSES . " eita, " . TABLE_EQUIPMENT . " e where eita.equipment_id = e.equipment_id and eita.equipment_status_id = '2' and e.equipment_type_id = '2' and eita.address_id = '" . tep_fill_variable('address_id', 'session') . "'");
																		...
																		$form['isntall_riders'] = $install_riders;	
						
											}
											elseif ($sc_reason == '2') {
												//Install new bb or rider.
												$vars['equipment_array'] = tep_generate_available_equipment_string_bgdn('1', tep_get_service_level_id($agent_id), $agent_id, $optional, fetch_address_zip4(tep_fill_variable('address_id', 'session')), tep_fill_variable('address_id', 'session'), false, true, array(), false);
						...
						$pulldowns['sc_reason'] = tep_generate_service_call_pulldown_menu_bgdn('sc_reason', $sc_reason, $agent_id, tep_fill_variable('address_id', 'session'), 'change-submit');
					...
					$vars['order_type'] = $order_type;
					$vars['step'] = 2;
					echo $twig->render('aom/aom_order_create_special_service.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'vars'=>$vars));
