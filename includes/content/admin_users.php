...
$with_balance = tep_fill_variable('with_balance', 'get', 'any');


if($show_user_group_id != 1 && $show_user_group_id != 4){
    $with_balance = 'any';
}
$message = '';
...
else if ($page_action == 'inactive_change_status')
{
	$database->query("update " . TABLE_USERS . " set active_status = '" . tep_fill_variable('status', 'get') . "' where user_id = '" . $uID . "' limit 1");
	$page_action = '';
	$uID = '';
...
<table width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
			...
			<?php
                ...
                <table id="myTable" width="100%" class="pageBox" cellspacing="0" cellpadding="2">
                    ...
                    foreach ($query as $result) {
                        ...
                        <tr>
                            <td width="20%"
                                class="pageBoxContent"><?php echo $result['firstname'] . ' ' . $result['lastname']; ?></td>
                            <td width="20%" class="pageBoxContent"
                                align="center"><?php echo $result['email_address']; ?></td>
                            <td width="20%" class="pageBoxContent"
                                align="center"><?php if ($result['active_status'] == '1') { ?><img
                                        src="images/icon_status_green.gif" height="10" width="10"
                                        border="0"/>&nbsp;&nbsp;<a
                                        href="<?php echo FILENAME_ADMIN_USERS . '?uID=' . $result['user_id'] . '&page_action=inactive_change_status&status=0&' . tep_get_all_get_params(array('page_action', 'action', 'uID')); ?>">
                                        <img src="images/icon_status_red_light.gif" height="10" width="10" border="0"/>
                                    </a><?php } else { ?><a
                                    href="<?php echo FILENAME_ADMIN_USERS . '?uID=' . $result['user_id'] . '&page_action=inactive_change_status&status=1&' . tep_get_all_get_params(array('page_action', 'action', 'uID', 'status')); ?>">
                                        <img src="images/icon_status_green_light.gif" height="10" width="10"
                                             border="0"/></a>&nbsp;&nbsp;<img src="images/icon_status_red.gif"
                                                                              height="10" width="10"
                                                                              border="0"/><?php } ?></td>
                            ...
                            <td width="90" class="pageBoxContent" align="right"><a
                                        href="<?php echo FILENAME_ADMIN_USERS . '?uID=' . $result['user_id'] . '&page_action=edit&' . tep_get_all_get_params(array('page_action', 'action', 'uID')); ?>">Edit</a>
                                |
                                <a href="<?php echo FILENAME_ADMIN_USERS . '?uID=' . $result['user_id'] . '&page_action=delete&' . tep_get_all_get_params(array('page_action', 'action', 'uID')); ?>">Delete</a><?php if ($result['user_group_id'] == '1') { ?>
                                    <br/><a
                                        href="<?php echo FILENAME_ADMIN_ORDERS . '?agent_id=' . $result['user_id']; ?>&order_status=">
                                        Orders</a><?php } ?><?php if (!empty($promo_code_result['count'])) { ?><br/><a
                                        href="<?php echo FILENAME_ADMIN_PROMO_TRACK . '?user_id=' . $result['user_id']; ?>">
                                        View Used Promo Codes</a><?php } ?> | <a
                                        href="<?php echo FILENAME_ADMIN_USERS . '?uID=' . $result['user_id'] . '&page_action=login&' . tep_get_all_get_params(array('page_action', 'action', 'uID')); ?>">Login</a>
                            </td>
                ...
                <?php
            }else if ($page_action != 'edit' && $page_action != 'add') {
			?>
		...
		#-- @End count Total Number of Agents to whom Inventory email send		
                ...
                }else if($with_balance == 'credit'){
                    $listing_split = new split_page("select u.user_id, u.email_address, ud.firstname, ud.lastname, ug.name, ug.user_group_id, u.agency_id, u.active_status, u.accounts_payable, u.service_level_id, u.order_hold, u.personal_invoice, ap.install_preference, ap.service_call_preference, ap.removal_preference from " . TABLE_USER_GROUPS . " ug, ". TABLE_ACCOUNTS . " ta, " . TABLE_USERS . " u left join " . TABLE_AGENCYS . " a on (u.agency_id = a.agency_id) left join agent_preferences ap ON ap.user_id=u.user_id, " . TABLE_USERS_DESCRIPTION . " ud, " . TABLE_USERS_TO_USER_GROUPS . " utug where u.user_id = ta.user_id and ta.running_total > 0 and u.user_id = ud.user_id and u.user_id = utug.user_id and u.users_status=1 and ".((!empty($show_user_group_id)) ? (($show_user_group_id == '5') ? "(utug.user_group_id = '" . $show_user_group_id . "' or u.accounts_payable = '1') and " : "utug.user_group_id = '" . $show_user_group_id . "' and ") : '').((!empty($search_name)) ? ("((ud.firstname like '" . $search_name . "%' or ud.firstname = '" . $search_name . "' or ud.firstname like '%" . $search_name . "' or ud.firstname like '%" . $search_name . "%') or (ud.lastname = '" . $search_name . "' or ud.lastname like '" . $search_name . "%' or ud.lastname like '%" . $search_name . "' or ud.lastname like '%" . $search_name . "%')) and ") : '').((!empty($search_email)) ? ("(u.email_address like '" . $search_email . "%' or u.email_address = '" . $search_email . "' or u.email_address like '%" . $search_email . "' or u.email_address like '%" . $search_email . "%') and ") : '').((!empty($search_mrsid)) ? (($search_mrsid == 'none') ? "u.agent_id = '' and " : "(u.agent_id like '" . $search_mrsid . "%' or u.agent_id like '%" . $search_mrsid . "%' or u.agent_id like '%" . $search_mrsid . "' or u.agent_id = '" . $search_mrsid . "') and ") : '')."utug.user_group_id = ug.user_group_id" . ((!empty($show_agency_id)) ? " and (a.agency_id = '" . $show_agency_id . "' or a.parent_agency_id = '" . $show_agency_id . "')" : '') . ((!empty($start_letter)) ? (($start_letter_type == 'any') ? " and (ud.firstname like '".$start_letter."%' or ud.lastname like '".$start_letter."%')" : (($start_letter_type == 'first') ? " and (ud.firstname like '".$start_letter."%')" : " and (ud.lastname like '".$start_letter."%')")) : '') . ((!empty($show_service_level_id)) ? " and (utug.user_group_id != '1' or u.service_level_id = '" . $show_service_level_id . "')" : '') . ((!empty($search_status)) ? (($search_status == '1') ? " and (u.active_status = '1') " : " and (u.active_status = '0') ") : '') . ((($billing_method != 'any')) ?  " and u.billing_method_id = '" . (int)$billing_method . "' " : '')  . (($show_user == 'with') ? (" and (ap.install_preference != '' or ap.service_call_preference != '' or ap.removal_preference != '')") : '') . (($show_user == 'without') ? (" and ap.install_preference='' and ap.service_call_preference='' and ap.removal_preference=''") : '') . " order by ud.lastname, ud.firstname", '20', 'u.user_id');
                }else if($with_balance == 'debit'){
                    $listing_split = new split_page("select u.user_id, u.email_address, ud.firstname, ud.lastname, ug.name, ug.user_group_id, u.agency_id, u.active_status, u.accounts_payable, u.service_level_id, u.order_hold, u.personal_invoice, ap.install_preference, ap.service_call_preference, ap.removal_preference from " . TABLE_USER_GROUPS . " ug, ". TABLE_ACCOUNTS . " ta, " . TABLE_USERS . " u left join " . TABLE_AGENCYS . " a on (u.agency_id = a.agency_id) left join agent_preferences ap ON ap.user_id=u.user_id, " . TABLE_USERS_DESCRIPTION . " ud, " . TABLE_USERS_TO_USER_GROUPS . " utug where u.user_id = ta.user_id and ta.running_total < 0 and u.user_id = ud.user_id and u.user_id = utug.user_id and u.users_status=1 and ".((!empty($show_user_group_id)) ? (($show_user_group_id == '5') ? "(utug.user_group_id = '" . $show_user_group_id . "' or u.accounts_payable = '1') and " : "utug.user_group_id = '" . $show_user_group_id . "' and ") : '').((!empty($search_name)) ? ("((ud.firstname like '" . $search_name . "%' or ud.firstname = '" . $search_name . "' or ud.firstname like '%" . $search_name . "' or ud.firstname like '%" . $search_name . "%') or (ud.lastname = '" . $search_name . "' or ud.lastname like '" . $search_name . "%' or ud.lastname like '%" . $search_name . "' or ud.lastname like '%" . $search_name . "%')) and ") : '').((!empty($search_email)) ? ("(u.email_address like '" . $search_email . "%' or u.email_address = '" . $search_email . "' or u.email_address like '%" . $search_email . "' or u.email_address like '%" . $search_email . "%') and ") : '').((!empty($search_mrsid)) ? (($search_mrsid == 'none') ? "u.agent_id = '' and " : "(u.agent_id like '" . $search_mrsid . "%' or u.agent_id like '%" . $search_mrsid . "%' or u.agent_id like '%" . $search_mrsid . "' or u.agent_id = '" . $search_mrsid . "') and ") : '')."utug.user_group_id = ug.user_group_id" . ((!empty($show_agency_id)) ? " and (a.agency_id = '" . $show_agency_id . "' or a.parent_agency_id = '" . $show_agency_id . "')" : '') . ((!empty($start_letter)) ? (($start_letter_type == 'any') ? " and (ud.firstname like '".$start_letter."%' or ud.lastname like '".$start_letter."%')" : (($start_letter_type == 'first') ? " and (ud.firstname like '".$start_letter."%')" : " and (ud.lastname like '".$start_letter."%')")) : '') . ((!empty($show_service_level_id)) ? " and (utug.user_group_id != '1' or u.service_level_id = '" . $show_service_level_id . "')" : '') . ((!empty($search_status)) ? (($search_status == '1') ? " and (u.active_status = '1') " : " and (u.active_status = '0') ") : '') . ((($billing_method != 'any')) ?  " and u.billing_method_id = '" . (int)$billing_method . "' " : '')  . (($show_user == 'with') ? (" and (ap.install_preference != '' or ap.service_call_preference != '' or ap.removal_preference != '')") : '') . (($show_user == 'without') ? (" and ap.install_preference='' and ap.service_call_preference='' and ap.removal_preference=''") : '') . " order by ud.lastname, ud.firstname", '20', 'u.user_id');
                }
						if ($listing_split->number_of_rows > 0) {
							$query = $database->query($listing_split->sql_query);
		...
		?>
			...
			<td class="main">Only Status: <select name="search_status"><option value=""<?php echo (($search_status == '') ? ' SELECTED' : ''); ?>>Any</option><option value="1"<?php echo (($search_status == '1') ? ' SELECTED' : ''); ?>>Active</option><option value="2"<?php echo (($search_status == '2') ? ' SELECTED' : ''); ?>>Inactive</option></select></td>
				...
				<?php if($show_user_group_id == 1 || $show_user_group_id == 4) { ?>
                    <tr>
                        <td class="main">
                            Balance
                            <select name="with_balance" id="with_balance">
                                <option value="any" <?php echo ($with_balance == 'any' ? ' SELECTED' : ''); ?>>Any</option>
                                <option value="credit" <?php echo ($with_balance == 'credit' ? ' SELECTED' : ''); ?>>Credit</option>
                                <option value="debit" <?php echo ($with_balance == 'debit' ? ' SELECTED' : ''); ?>>Debit</option>
                            </select>
                        </td>
                    </tr>
                    <?php } ?>				  
