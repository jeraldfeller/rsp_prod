...
@session_start(); 
	...
	$order = new orders('fetch', $order_id);	$data = $order->fetch_order();	$base_cost = $data['base_cost'];	$extended_cost = $data['extended_cost'];	$equipment_cost = $data['equipment_cost'];	$extra_cost = $data['extra_cost'];	$deposit_cost = $data['deposit_cost'];	$discount_cost = $data['discount_cost'];	$total = $data['order_total'];    $credit = tep_fill_variable('credit', 'session', 0);    $total -= $credit;		
...
//	$form['sc']['name'] = $string;
  ...
  $form['sc']['name'] = $string;
	...
	$vars['step'] = 5;	 
	echo $twig->render('order/order_create_success.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'order'=>$order, 'vars'=>$vars));
?>
