...
if (!empty($submit_value)) {
	...
	if (!$error->get_error_status('admin_users')) 
	{
		...
		else 
		{
			...
			$database->query("insert into " . TABLE_USERS . " (email_address, password, agent_id, billing_method_id, service_level_id, agency_id, require_deposit, deposit_remaining_count, discount_type, discount_amount, accounts_payable, personal_invoice) values ('" . $email_address . "', '" . md5($password) . "', '" . $agent_id . "', '" . $billing_method_id . "', '" . $service_level_id . "', '" . $agency_id . "', '" . $require_deposit . "', '" . $deposit_remaining_count . "', '" . $discount_type . "', '" . $discount_amount . "', '" . $accounts_payable . "', '" . $personal_invoice . "')");
