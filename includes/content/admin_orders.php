<style>
    .rsp_button{
        ...
        background-color: #6EB9E3; background-image: -webkit-gradient(linear, left top, left bottom, from(#6EB9E3), to(#DEECF0));
        background-image: -webkit-linear-gradient(top, #6EB9E3, #DEECF0);
        background-image: -moz-linear-gradient(top, #6EB9E3, #DEECF0);
        background-image: -ms-linear-gradient(top, #6EB9E3, #DEECF0);
        background-image: -o-linear-gradient(top, #6EB9E3, #DEECF0);
        background-image: linear-gradient(to bottom, #6EB9E3, #DEECF0);filter:progid:DXImageTransform.Microsoft.gradient(GradientType=0,startColorstr=#6EB9E3, endColorstr=#DEECF0);
    }
</style>
<?php
        ...
        if (!$error->get_error_status('admin_orders')) {
            ...
            if (($auto_removal == '1') && ($auto_remove_period > 0)) {
                ...
                $data = array('address_id' => $address_id,
                    ...
                    'county' => $county);
                $order = new orders('insert', '', $data, $user_id, true, '2');
				$totalQuery = $database->query("SELECT order_total FROM orders WHERE order_id = ". $order_id ."");
                $total = $database->fetch_array($totalQuery);
                if(isset($_POST['button_action_ioacd'])){
                    Header('Location: admin_accounts.php?billing_method=3&agent_id='.$user_id.'&total='.$total['order_total']);
                }
            }
        } else {
            $page_action = 'add';
...
?>
                        <tr>
                            ...
                            <tr>
							<td class="main">State </td><td><?php echo tep_draw_state_pulldown('state', $state, ''); ?></td>
                            </tr>
                            <tr>
                                <td class="main">Zip Code: </td><td><input type="text" name="zip" value="<?php echo $zip; ?>"/></td>
                            </tr>
                            <tr>
                                                ...
                                                foreach($query as $result)
                                                    {
                                                        ...
                                                        foreach($sub_query as $sub_result){
...
$ioacdDisabled = 'disabled';
    if($order_type_id == 1 || $order_type_id == 2){
        if($user_id != ''){
            $query = $database->query("SELECT billing_method_id FROM ". TABLE_USERS." WHERE user_id = ".$user_id." AND billing_method_id = 1");
            $result = $database->fetch_array($query);
                        ...
                        <table width="250" cellspacing="0" celpadding="0" class="pageBox">
                            ...
                            <tr>
                                <td width="100%" align="right">
                                    <table width="100%" cellspacing="0" cellpadding="0">
                                        <tr>
                                            <td align="left"><?php echo tep_create_button_submit('insert_order', 'Insert Order', ' name="button_action"'); ?></td>
                                            <td align="center"><input type="submit" class="rsp_button" name="button_action_ioacd" value="Insert Order and Create Deferred CC Charge" <?php echo $ioacdDisabled; ?>/></form></td>
                                            <td align="right"><form action="<?php echo FILENAME_ADMIN_ORDERS.'?'.tep_get_all_get_params(array('oID', 'action', 'page_action')); ?>" method="post"><?php echo tep_create_button_submit('cancel', 'Cancel'); ?></form></td>
                                        </tr>
                                    </table>
                                </td>
