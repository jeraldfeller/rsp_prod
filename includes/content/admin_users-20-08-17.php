...
if (!empty($submit_value)) {
	...
	if (!$error->get_error_status('admin_users')) 
	{
		...
		else 
		{
			...
			$database->query("insert into " . TABLE_USERS . " (email_address, password, agent_id, billing_method_id, service_level_id, agency_id, require_deposit, deposit_remaining_count, discount_type, discount_amount, accounts_payable, personal_invoice, use_address) values ('" . $email_address . "', '" . md5($password) . "', '" . $agent_id . "', '" . $billing_method_id . "', '" . $service_level_id . "', '" . $agency_id . "', '" . $require_deposit . "', '" . $deposit_remaining_count . "', '" . $discount_type . "', '" . $discount_amount . "', '" . $accounts_payable . "', '" . $personal_invoice . "', '".$use_address."')");
			...
			if ($message[$y] != '') {		
				...
				*/		
				...
				$new_message = "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\">		
					<tbody>		
						<tr>		
							<td>Greetings from Realty SignPost. This e-mail is for ".$name[$y]." with ".$agency[$x]." at ".$agency_addr[$x].". If you are now with a different Agency or the Agency is at a different address, please let us know and we will make the appropriate updates.<br>		
							Below is a list of addresses where you currently have a signpost installed. This list includes the date the signpost was installed, and the scheduled removal date, if a removal date has been scheduled for that signpost. 		
							...
							<br /><br />		
							If you feel there are mistakes on this list, please contact us at <a href='".INFO_EMAIL."'>".INFO_EMAIL."</a>, and we will make the appropriate corrections.		
							</td>		
						</tr>		
						<tr>		
							<td>&nbsp;</td>		
...
if ($page_action == 'login_confirm') 
{
	...
	if($user->user_group_id == 1) {
		...
		tep_redirect(HTTP_PREFIX . '/' .'aom_active_addresses.php');
								...
								$queryPage = $database->query("select page_url from ".TABLE_USERS." where user_id='$uID'");
	...
	<tr>
		...
		?>
			...
			<table width="250" cellspacing="0" celpadding="0" class="pageBox">
				...
				<tr>		
					<td width="100%" align="right"><form name="admin_config" method="post" action="<?php echo FILENAME_ADMIN_USERS . '?page_action=sendemails&'. tep_get_all_get_params(array('page_action', 'action', 'uID')); ?>"><input type="submit" value="Send Emails"></form></td>
				</tr>
			</table>
		<?php
				}
