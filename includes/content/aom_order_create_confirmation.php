...
$selected_agent_id = tep_fill_variable('agent_id', 'session');$aom_id = tep_fill_variable('user_id', 'session');	
  ...
  if (!empty($page_action) && ($page_action == 'submit')) {
    ...
    if (!$error->get_error_status('order_create_confirmation')) {
      ...
      if (($payment_method == BILLING_METHOD_CREDIT) && ($total + $deferred_total > 0)){
        ...
        $errorFlag = true;
        if (empty($error_code)) {
								$agency_query = $database->query("select a.name, a.service_level_id, a.billing_method_id, a.address, a.contact_name, a.contact_phone from " . TABLE_USERS . " u, " . TABLE_AGENCYS . " a where u.user_id = '" . $selected_agent_id . "' and u.agency_id = a.agency_id limit 1");
        ...
        if ($errorFlag) {
          ...
          tep_redirect(FILENAME_AOM_ORDER_CREATE_PAYMENT);
            ...
            $error->cc_error("Not credit $user_name($selected_agent_id) $house_number $street_name for $total");
      }
      if (empty($address_id)) 
	  {
      ...
      $data = array('address_id' => tep_fill_variable('address_id', 'session'),
        'order_type_id' => tep_fill_variable('order_type_id', 'session'),		'user_id' => $selected_agent_id,
        'schedualed_start' => tep_fill_variable('schedualed_start', 'session'),
        'special_instructions' => tep_fill_variable('special_instructions', 'session'),
        'optional' => $optional,
        'number_of_posts' => $number_of_posts,
      ...
      if (($auto_remove_period > 0) && (tep_fill_variable('order_type_id', 'session') == ORDER_TYPE_INSTALL)) {
        ...
        $data = array('address_id' => tep_fill_variable('address_id', 'session'),
          'order_type_id' => ORDER_TYPE_REMOVAL,		  'user_id' => $selected_agent_id,
...
$form = array (
 ...
 $vars['agent_data'] = tep_fetch_agent_data($selected_agent_id);  //print_r( $vars['agent_data']);
...
if ($user->fetch_billing_method_id() == 1 && $deferred_total != 0) {
    $deferred = $deferred->createSiteHTMLTwig($total);
} else {
	$deferred = null;
}
...
$vars['deferred'] = $deferred;
	...
	echo $twig->render('aom/aom_order_create_confirmation.html.twig', array('user' => $user, 'page' => $page, 'error'=>$error, 'order'=>$order, 'vars'=>$vars));
